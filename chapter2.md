...
-  С помощью функции `ggplot2()` постройте эмпирическую функцию распределен. Все Ваши предыдущие расчеты уже находятся в рабочем пространстве. Вам нужно указать только оси:  `mapping = aes(x=  , y=   )`.
