...
`@sample_code`
```{r}
#Оцека среднего времени ожидания между извержениями гейзера с использованием первого начального момента.
mean_time <- 
...
#General
test_error()
success_msg("Отлично! Так держать! 0.75-квантиль - красная линия на изображенном графике, левее которой площадь под графиком функции плотности равна 0.75.")
```
