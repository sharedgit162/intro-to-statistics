...
- Посчитайте критические значения и сохраните их в переменный `crit_low` и `crit_high`. Не забывайе, что они не симметричны, как это было в случае стандартного нормального распределения.
...
msg_3 = "Для вычисления верхней границы, поделите произведение выборочной дисперсии и числа степеней свободы на меньшее критическое значение."
msg_4 = "Для вычисления нижней границы, поделите произведение выборочной дисперсии и числа степеней свободы на большее критическое значение."
...
#Постройте выборочную функцию распределения
ggplot(mapping = aes(x=___ , y=___)) + geom_line() + labs(title = 'Эмпирическая функция распределения', x = 'Время ожидания', y = 'F(x)')+theme(plot.title = element_text(hjust = 0.5))
```
`@solution`
```{r}
#Загрузите пакет для визуализации данных
