...
$$Var(X) = E(X - E(X))^2$$
  ...
  Дополните код справа и сохраните полученное значение в переменную `sigma_2`. Выведите её.
...
ex() %>% check_object("sigma_2") %>% check_equal(incorrect_msg = "Проверьте правильность переменной `sigma_2`.")
