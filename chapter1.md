...
* Постройте график функции плотности Хи-квадрат распределения с 3 степенями свободы. Пакет `tidyverse` для Вас уже установлен. Воспользуйтесь уже знакомой командой `ggplot`, построить функцию можно с помощью команды `stat_function`.
...
test_error()
success_msg("Замечательно! Как Вы могли убедиться, ")
```
