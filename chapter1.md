...
* С помощью интегрирования посчитайте дисперсию распределения Стьюдента с 3 степенями свободы. Для этого вспомните формулу для нахождения дисперсии, которую Вы использовали в предыдущем блоке: $Var(X) = E(X^2) - (E(X))^2$. 
*  Используя уже знакомые Вам команды `ggplot` и `stat_function`, на одном графике постройте функции плотности для распределения Стьюдента с 2, 5 и 50 степенями свободы. (пакет `ggplot2` для Вас уже установлен).
...
#General
test_error()
success_msg("Отлично! Вы могли заметить, что получившееся в результате значение дисперсии эквивалентно вычисленному по формуле $/frac{n}{n - 2}$, где n - числе степеней свободы.")
```
