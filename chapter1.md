...
* Вычислите дисперсию всех результатов эксперимента. Убедитесь, что она равна $np(1-p)$. Присвойте переменной `var_all` полученное значение.

* Выведите стандартное отклонение всех результатов эксперимента. Команда `sqrt()` может в этом пригодиться. 
