...
* Посчитайте моду Хи-квадрат распределения с 3 степенями свободы. Для этого необходимо создать функцию плотности распределения f, используя команды `function(x)` и `dchisq(x, df)`. После это максимизируйте данную функцию плотности с помощью команды `optimize` (по умолчанию данная команда минимизирует функцию, поэтому необходимо поставить значение `TRUE` аргумента `maximize`).
...
---
## Характеристики многомерного распределения
...
В этом задании Вам предстоит помочь исследователю Геннадию вычислить некоторые характеристики случайных величин со следующей функцией распределения: $$f(x, y)=\begin{array}{c l}
     4xy,  \ 0<x<1,  \ 0<y<1,\\
      0,& \text{иначе.}
\end{array}$$
Вам необходимо посчитать ковариацию случайных величин $X$ и $Y$, для этого вспомните формулу: $cov(X, Y,)  = E(XY) - E(X)E(Y).$ 

`@instructions`
*  Посчитайте математическое ожидание $E(X)$, для этого воспользуйтесь командами `function`, а также `int2(f, a=c(), b=c())`, где `a` и `b` обозначают нижние и верхние пределы интегрирования, соответственно, (пакет `rmutil`, необходимый, чтобы взять двойной интеграл, уже установлен). Чтобы провести все расчеты, вспомните формулу математического ожидания для непрерывного случая и нескольких случайных величин:  ![](https://dl.dropboxusercontent.com/s/7qvscpa8ykkqkef/1234.png?dl=0).
* Повторите аналогичные расчеты для $E(Y)$.
* Теперь у Вас есть все необходимые значения для расчета ковариации по формуле. Для удобства вычисления воспользуйтесь командой `round(x, digits=0)`, где `digits` означает количество знаков после запятой.
...
round(___, 2) - round(___, 2)*round(___, 2)
```
`@solution`
```{r}
f <- function(x, y) {
...
round(mu_xy, 2) - round(mu_x, 2)*round(mu_y, 2)
```
`@sct`
```{r}
#General
...
test_error()
success_msg("Вы замечательно постарались! Подумайте, о чем говорит полученное значение ковариации.")
```
