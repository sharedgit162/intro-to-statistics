...
`@feedback`
- Верно! Мы не можем точно сказать, какое значение покажет прибор до его включения. Это значение случайно.
- Мы не можем точно сказать, какое значение покажет прибор до его включения. Это значение случайно.
- Геннадий точно знает, что именно показал прибор. Это значение не случайно.
- Вне зависимости от времени, Геннадий точно знает, что именно показал прибор. Это значение не случайно.
...
Рассчитайте дисперсию результата одного падения, дисперсию всех результатов эксперимента, а также стандартное отклонение результатов всех падений, используя консоль. Выберите верный ответ.
...
* Посчитайте математическое ожидание Хи-квадрат распределения с 3 степенями свободы. Для этого вспомните формулу математического ожидания в непрерывном случае: $E(X) = \int_{-/infty}^{+/infty} x f_X (x) dx$, а также команды `function(x)` и `dchisq(x, df)`, с которыми Вы уже знакомы. Интегрировать функцию поможет команда `integrate(f, lower, upper)`.
* Постройте график функции плотности Хи-квадрат распределения с 3 степенями свободы. Пакет `ggplot2` для Вас уже установлен. Воспользуйтесь уже знакомой командой `ggplot`, построить функцию можно с помощью команды `stat_function`.
