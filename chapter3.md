...
- Сохраните полученный результат в переменную `mean_time`.
...
```{r}
#first instruction
test_function("mean", not_typed_msg = "Что-то не так с `mean_time`. Вызывали ли вы функцию `mean()` для подсчета среднего?")
